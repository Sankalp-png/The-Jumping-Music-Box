 ...
 *
 * p5.play is not a ball2D-derived physics engine, it doesn't use events, and it's
...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  * It can either be an Axis Aligned Bounding ball (a non-rotated rectangle)
  ...
  */
   ...
   * placeholder rectangle or the bounding ball of the image.
   *
  ...
  *
  * Often the image bounding ball is not appropriate as the active area for
  ...
  *
  * * `aabb` - An axis-aligned bounding ball - has width and height but no rotation.
  *
  *   `setCollider("aabb"[, offsetX, offsetY[, width, height]])`
  *
  * * `obb` - An oriented bounding ball - has width, height, and rotation.
  *
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height, rotation) {
    ...
    if (_type === 'point') {
      ...
      this.collider = p5.AxisAlignedBoundingballCollider.createFromSprite(this, offset, width, height);
    } else if (_type === 'obb') {
      this.collider = p5.OrientedBoundingballCollider.createFromSprite(this, offset, width, height, radians(rotation));
    }

    this._sweptCollider = new p5.OrientedBoundingballCollider();
  ...
  * they will be created automatically from the image/animation bounding ball.
  *
...
Quadtree.prototype.getIndex = function( pRect ) {
  ...
  {
    var colliderBounds = pRect.collider.getBoundingball();
    var index 				= -1,
...
p5.prototype._warn = function(message) {
  ...
  p5.CircleCollider.prototype._getCandidateAxes = function(other) {
    ...
    if (other instanceof p5.OrientedBoundingballCollider || other instanceof p5.AxisAlignedBoundingballCollider) {
      // There are four possible separating axes with a ball - one for each
  ...
  /**
   * An Axis-Aligned Bounding ball (AABB) collision shape, used to detect overlap
   ...
   *
   * @class p5.AxisAlignedBoundingballCollider
   * @constructor
   * @extends p5.CollisionShape
   * @param {p5.Vector} center
   * @param {number} width
   * @param {number} height
   */
  p5.AxisAlignedBoundingballCollider = function(center, width, height) {
    p5.CollisionShape.call(this, center);
    ...
    Object.defineProperties(this, {
      ...
      /**
       * The untransformed width of the ball collider.
      ...
      /**
       * The unrotated height of the ball collider.
       * Recomputes diagonals when set.
       * @property height
       * @type {number}
       */
      ...
      /**
       * Two vectors representing adjacent half-diagonals of the ball at its
  ...
  p5.AxisAlignedBoundingballCollider.prototype = Object.create(p5.CollisionShape.prototype);

  /**
   * Construct a new AxisAlignedBoundingballCollider with given offset for the given sprite.
   * @method createFromSprite
   * @static
   * @param {Sprite} sprite
   * @param {p5.Vector} [offset] from the sprite's center
   * @return {p5.CircleCollider}
   */
  p5.AxisAlignedBoundingballCollider.createFromSprite = function(sprite, offset, width, height) {
    var customSize = typeof width === 'number' && typeof height === 'number';
    var ball = new p5.AxisAlignedBoundingballCollider(
      offset,
      customSize ? width : 1,
      customSize ? height : 1
    );
    ball.getsDimensionsFromSprite = !customSize;
    ball.updateFromSprite(sprite);
    return ball;
  };
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.AxisAlignedBoundingballCollider.prototype.updateFromSprite = function(sprite) {
    if (this.getsDimensionsFromSprite) {
      if (sprite.animation) {
   ...
   * @protected
   */
  p5.AxisAlignedBoundingballCollider.prototype._onTransformChanged = function() {
    p5.CollisionShape.prototype._onTransformChanged.call(this);
    this._computeHalfDiagonals();
  };

  /**
   * Recompute this bounding ball's half-diagonal vectors.
   * @method _computeHalfDiagonals
   * @private
   * @return {Array.<p5.Vector>}
   */
  p5.AxisAlignedBoundingballCollider.prototype._computeHalfDiagonals = function() {
    // We transform the rectangle (which may scale and rotate it) then compute
    // an axis-aligned bounding ball _around_ it.
   ...
   */
  p5.AxisAlignedBoundingballCollider.prototype.draw = function(sketch) {
    sketch.push();
    sketch.rectMode(sketch.CENTER);
    sketch.translate(this.center.x, this.center.y);
    sketch.noFill();
   ...
   * @return {Array.<p5.Vector>}
   */
  p5.AxisAlignedBoundingballCollider.prototype._getCandidateAxes = function() {
   ...
   * @return {number}
   */
  p5.AxisAlignedBoundingballCollider.prototype._getRadiusOnAxis = function(axis) {
   ...
   * @return {number}
   */
  p5.AxisAlignedBoundingballCollider.prototype._getMinRadius = function() {
    return Math.min(this._width, this._height);
  };

  /**
   * An Oriented Bounding ball (OBB) collision shape, used to detect overlap and
   * compute minimum displacement vectors with other collision shapes.
   * @class p5.OrientedBoundingballCollider
   * @constructor
   * @extends p5.CollisionShape
   ...
   */
  p5.OrientedBoundingballCollider = function(center, width, height, rotation) {
    ...
    Object.defineProperties(this, {
      ...
      /**
       * The unrotated width of the ball collider.
       * Recomputes diagonals when set.
       * @property width
       * @type {number}
       */
  ...
  p5.OrientedBoundingballCollider.prototype = Object.create(p5.CollisionShape.prototype);
   ...
   * @return {p5.CircleCollider}
   */
  p5.OrientedBoundingballCollider.createFromSprite = function(sprite, offset, width, height, rotation) {
    var customSize = typeof width === 'number' && typeof height === 'number';
    var ball = new p5.OrientedBoundingballCollider(
      offset,
      customSize ? width : 1,
      customSize ? height : 1,
      rotation
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.OrientedBoundingballCollider.prototype.updateFromSprite =
    p5.AxisAlignedBoundingballCollider.prototype.updateFromSprite;
   ...
   * @param {Sprite} sprite
   */
  p5.OrientedBoundingballCollider.prototype.updateSweptColliderFromSprite = function(sprite) {
   ...
   * @protected
   */
  p5.OrientedBoundingballCollider.prototype._onTransformChanged = function() {
    p5.CollisionShape.prototype._onTransformChanged.call(this);
   ...
   * @param {p5} sketch - p5 instance to use for drawing
   */
  p5.OrientedBoundingballCollider.prototype.draw = function(sketch) {
   ...
   * @return {Array.<p5.Vector>}
   */
  p5.OrientedBoundingballCollider.prototype._getCandidateAxes = function() {
    // An oriented bounding ball always provides two of its face normals,
   ...
   * @return {number}
   */
  p5.OrientedBoundingballCollider.prototype._getRadiusOnAxis =
    p5.AxisAlignedBoundingballCollider.prototype._getRadiusOnAxis;
  // We can reuse the AABB version of this method because both are projecting
  // cached half-diagonals - the same code works.

  /**
   * When checking for tunneling through a OrientedBoundingballCollider use a
   ...
   * @return {number}
   */
  p5.OrientedBoundingballCollider.prototype._getMinRadius =
    p5.AxisAlignedBoundingballCollider.prototype._getMinRadius;
